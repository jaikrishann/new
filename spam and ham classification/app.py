...
# (used here to load the pre-trained model and vectorizer).
...
# vectorizer: The vectorizer used to transform text data into numerical
#  format,
...
app = Flask(__name__)
# app: An instance of the Flask class, which is the main 
# object for your web application.
@app.route('/', methods=['GET', 'POST'])
def home():
    return render_template('index.html')
# @app.route('/'): Decorator to define the URL route
#  for the home page (root URL).
# methods=['GET', 'POST']: The home route can handle both 
# GET and POST requests.
# home(): Function to handle requests to the home route,
#  rendering the 'index.html' template.
# The render_template function in Flask is used to render
#  an HTML template and
# return it as a response to the client. This function
...
# @app.route('/predict', methods=['POST']): Decorator to 
# define the URL route for the prediction functionality.
...
# messg = str(request.form['mesg']): Retrieves the message text 
# from the form data.
# transformed = vectorizer.transform([messg]): Transforms the message text 
# into a numerical format using the vectorizer.
# transformed_data = transformed.toarray(): Converts the transformed 
# data to an array format.
